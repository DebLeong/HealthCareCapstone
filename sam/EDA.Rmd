...
```{r}
# Make three get_Bnet functions for each combination
bnet = get_Bnet(data, c(1), c(100,200,300))
```
...
ggraph(bnet, layout = "kk")+
  ...
  geom_node_point(aes(fitler = size >= 10, color = fraud, size = size), shape = V(bnet)$shape) +
...
bb <- layout_as_backbone(bnet,keep = 0.3)
E(bnet)$col <- FALSE
E(bnet)$col[bb$backbone] <- TRUE
  
ggraph(bnet, layout = "manual",x = bb$xy[,1],y = bb$xy[,2])+
  geom_edge_link0(alpha=0.5, edge_colour = "grey66", aes(width = weights)) + 
  geom_node_point(aes(color = fraud, size = size), shape = V(bnet)$shape) +
