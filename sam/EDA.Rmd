...
data = docs %>% left_join(target, by=c('Provider','Set')) %>% select(-c('X1.x','X1.y'))
```
...
get_Bnet = function(data, state, county){
  ...
  data.filt = 
    data %>% 
    select(State, County, Provider, PotentialFraud, ClaimID, BeneID) %>% 
    group_by(State, County, Provider, PotentialFraud, BeneID) %>% 
    tally() %>% 
    filter(State %in% state, County %in% county) %>% 
    data.frame()
  
  df = get_Bnet %>% select(Provider,BeneID)
  Bnet <- graph.data.frame(df,directed=FALSE)
...
data_pdc = data %>% 
  select(State, County, Provider, PotentialFraud, ClaimID, AttendingPhysician, OperatingPhysician, OtherPhysician) %>% 
  pivot_longer(cols=c(AttendingPhysician,OperatingPhysician,OtherPhysician), names_to = "Type", values_to = "Doctor") %>% 
  filter(complete.cases(.))

matrix = data_pdc %>% 
  group_by(State, County, Provider, PotentialFraud, Doctor) %>% 
  tally() %>% 
  arrange(desc(n)) %>% 
  ungroup()
```
...
```{r}
data_pdc_5  = matrix %>% filter(State==5, County == 200) %>% data.frame()
```
...
```{r}
beneP  = data_beneP %>% filter(State==50, County == 200) %>% data.frame()
beneP
```
```{r}
nodes = beneP %>% 
  distinct(Provider, PotentialFraud) %>% 
...
fn4 <-
  function(DT, by_col) {
    temp <- merge(DT, DT, by = by_col, allow.cartesian = TRUE)
    temp[temp$B.x != temp$B.y , -1]
  }
```

## Finally Acheived a bipartite edge list
```{r}
edge.list = by(beneP, beneP$Provider, function(x) combn2(unlist(x$BeneID))) %>% map(data.frame)
edge.mat = map_dfr(edge.list, .f = cbind, .id = 'index') %>% tibble()
```
...
vertex.shape=shapes[V(bn)$type+1], vertex.size=10,vertex.label.degree=-pi/2, vertex.label.dist=1.2,vertex.label.cex=0.9, layout=lay)
```

```{r}
df = beneP %>% select(Provider,BeneID)
binet <- graph.data.frame(df,directed=FALSE)
binet
```


```{r}
fraud = beneP %>% select(Provider,PotentialFraud)
```

```{r}
V(binet)$type <- V(binet)$name %in% df[,1]
V(binet)$actor <- ifelse(V(binet)$name %in% df[,1],'Provider','Patient')

V(binet)$fraud <- ifelse(V(binet)$name %in% fraud[,1],fraud[,2],'')
V(binet)$color <- ifelse(V(binet)$fraud == 'Yes' ,'red',
                         ifelse(V(binet)$fraud == 'No','blue','black'))
...
ggraph(binet) +
  geom_edge_arc(color="gray50", width=0.8, alpha=0.5, curvature=0.3) + 
  geom_node_point(color = V(binet)$color) +
  theme_bw() # add nodes to the plot
```
```{r}
bn.pr <- bipartite.projection(binet)
...
ggraph(bn.provider) +
  geom_edge_arc(color="gray50", width=0.8, alpha=0.5, curvature=0.3) + 
  geom_node_point(aes(color = fraud)) +
  theme_bw() # add nodes to the plot
par(op)
```
```{r}
