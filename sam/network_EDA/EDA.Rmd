...
for (state in data$State %>% unique()) {
    ...
    # Calculate Mean Path Length to Each Node
    mean_paths = mean_path_length(bn.provider)
    mean_paths = tibble(names(mean_paths), mean_paths)
    colnames(mean_paths) = c('Provider','Mean.Path')
    pathlist[[state]] = mean_paths
    
    # Calculate Degrees
    degrees = degree(bn.provider)
    degrees = tibble(names(degrees), degrees)
    colnames(degrees) = c('Provider','NumDocNet')
    
    #data.agg = degrees %>% left_join(mean_paths, by='Provider')
    
    datalist[[state]] = degrees# add it to your list
}



propatDeg <- dplyr::bind_rows(datalist)
propatPath <- dplyr::bind_rows(pathlist)
...
prodocDeg <- dplyr::bind_rows(datalist)
prodocPath <- dplyr::bind_rows(pathlist)
...
write_csv(prodocNet,'../data/prodocNet.csv')
write_csv(propatNet,'../data/propatNet.csv')

prodocNet = prodocNet %>% group_by(Provider) %>% 
  summarise(max = max(NumDocNet), min = min(NumDocNet), mean = mean(NumDocNet))

propatNet = propatNet %>% group_by(Provider) %>% 
  summarise(max = max(NumBenNet), min = min(NumBenNet), mean = mean(NumBenNet))

colnames(propatNet) = c('Provider','patNet_max','patNet_min','patNet_mean')
colnames(prodocNet) = c('Provider','docNet_max','docNet_min','docNet_mean')
...
bnet = propat(data, c('Alabama'), 'all')
bn.pr = bipartite.projection(bnet)
bn.provider <- bn.pr$proj2
