...
prpat = function(data, state, county){
  ...
  data.filt = 
    ...
    dplyr::select(Provider, PotentialFraud, BeneID, Status, InscClaimAmtReimbursed) %>%
...
```{r}
bnet = prpat(data, c('New York'), 'all')
...
plot1 = plotActor(bn.patient, 'Patient', layout = 'circlepack', actor='patient')
plot2 = plotActor(bn.provider, 'Provider', layout = 'stress', actor='provider')
plot1 + plot2
plotActor(bn.provider, 'title', layout = 'tree', actor='doctor')
```
```{r}
ggraph(bn.provider, 'matrix', sort.by = node_rank_spectral()) + 
...
```
```{r}
data %>% filter(State %in% 'Oklahoma', County %in% 100) %>% 
