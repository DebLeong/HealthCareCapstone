...
for (state in data$State %>% unique()) {
    ...
    # Calculate Strength (Really a degree measure weighted by number of connections)
    ...
    # # Bundle Up
    # metrics = deg %>% 
    #   left_join(str, by = 'Provider') %>% 
    #   left_join(ann, by = 'Provider')
    
    deglist[[state]] = deg # add it to your list
    #strlist[[state]] = str # add it to your list
    annlist[[state]] = ann # add it to your list
}
#############################################################################

propatDeg <- dplyr::bind_rows(deglist)
#propatStr <- dplyr::bind_rows(strlist)
...
#prodocStr <- dplyr::bind_rows(strlist)
...
#prodocStr_clean = prodocStr %>% group_by(Provider) %>% summarise(docStrMax = max(Strength, na.rm=TRUE))
propatANN_clean = propatANN %>% group_by(Provider) %>% summarise(patMANN = max(Avg.NN, na.rm=TRUE))

prodocDeg_clean = prodocDeg %>% group_by(Provider) %>% summarise(docDegMax = max(Degree, na.rm=TRUE))
#prodocStr_clean = prodocStr %>% group_by(Provider) %>% summarise(docStrMax = max(Strength, na.rm=TRUE))
prodocANN_clean = prodocANN %>% group_by(Provider) %>% summarise(docMANN = max(Avg.NN, na.rm=TRUE))
